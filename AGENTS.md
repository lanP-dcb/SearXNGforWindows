...
| `limiter.py:141` | `limiter.toml` 同样改从 `os.getcwd()/config/` 加载 |
...
默认启用（`disabled: false`）的引擎只有 百度、Bing（含图片/新闻/视频）、搜狗（含图片/视频/微信）、SoundCloud，其余全部 `disabled: true`。`README.md` 中「仅保留搜狗和百度」的说法已过期。
...
若要引入上游测试，注意上游测试假设 cwd 是 git checkout 且 `searx/` 在根目录 —— 这正是本移植在 `settings_loader.py:36` 和 `limiter.py:141` 打破的假设。
